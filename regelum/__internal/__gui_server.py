...
def get_reports():
    return sorted(
        list(
            set(
                filter(
                    lambda name: name.startswith("report"),
                    [
                        os.fsencode(file).decode("utf-8").split(".")[0]
    ...
    try:
        ...
        while True:
            ...
            for _, report in enumerate(reports):
                if report not in containers:
                    ...
                    with shelve.open(report) as r:
                        with containers[report]["body"]:
                            ...
                            st.code(
                                f'file://{containers[report]["path"] + "/SUMMARY.html"}'
                            )
                            ...
                            containers[report]["button_clicked"] = containers[report][
                                "button_block"
                            ].button(
                                label="Stop",
                                on_click=Stopper(report),
                                key=f"button_{report}",
                            )
                ...
                if load_callbacks:
                    with open(
                        containers[report]["path"] + "/callbacks.dill", "rb"
                    ) as f:
                ...
                with shelve.open(report) as r:
                    if "traceback" in r:
                        ...
                        del r["traceback"]
                    containers[report]["episode_progress"].progress(
                        r["elapsed_relative"] if "elapsed_relative" in r else 0.0
                    )
                    if "episode_total" in r:
                        containers[report]["scenario_progress"].progress(
                            r["episode_current"] / r["episode_total"]
                        )
                        containers[report]["scenario_caption"].write(
                            f'Scenario ({r["episode_current"]}/{r["episode_total"]}):'
                        )
